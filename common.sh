...
func_schema_setup() {
  if [ "$schema_setup" == "mongo" ]; then
    ...
    mongo --host mongodb.saikrishna.cloud </app/schema/${component}.js &>>$log_file
    func_stat_check $?
  fi
  if [ "${schema_setup}" == "mysql" ]; then
    ...
    func_print_head "Load Schema"
    mysql -h mysql.saikrishna.cloud -uroot -p${mysql_root_password} < /app/schema/shipping.sql &>>$log_file
    func_stat_check $?
  fi
}
